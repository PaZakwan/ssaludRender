...
const HistorialMotivoSchema = new mongoose.Schema({
  createdAt: {
    type: Date,
    default: Date.now,
  },
  ...
  severidad: {
    ...
    required: [true, "La severidad es necesaria"],
  },
...
HistorialMotivoSchema.pre("save", async function (next) {
  ...
  if (this.estado === "Activo") {
    let motivoDB = await HistorialMotivo.find({
      ...
      .select("_id id")
      .exec();
    if (motivoDB?.some?.((motivo) => motivo._id !== this._id)) {
      throw new Error(
        `Ya existe un Mismo Motivo (Programa) "Activo", evolucione con consultas el existente.`
      );
    }
  }

  // throw new Error(`Testiando, deberia guardarlo.`);

  next();
});

// antes del update
HistorialMotivoSchema.pre(["findOneAndUpdate", "updateOne", "updateMany"], async function (next) {
  // No mismo Motivo Activo (paciente / estado : "Activo" / motivo_especialidad / descripcion)
  if (this.getUpdate().$set.estado === "Activo") {
    let motivoDB = await this.model
      .find({
      ...
      .lean()
      .exec();
    if (motivoDB?.some?.((motivo) => motivo._id != this.getUpdate().$set._id)) {
      throw new Error(
        `Ya existe un Mismo Motivo (Programa) "Activo", evolucione con consultas el existente.`
      );
    }
