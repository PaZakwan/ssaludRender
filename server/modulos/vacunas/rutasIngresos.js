...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // Ingresos
      let ingresosOrdenDB = await VacunaIngreso.aggregate()
        .match(filtro)
        .sort({fecha: -1, _id: -1})
        .addFields({
      ...
      let transferenciaRemitoDB = await VacunaTransferencia.aggregate()
        .match(filtro)
        .addFields({
          // fecha: {$dateToString: {format: "%Y-%m-%d", date: "$fecha"}},
  ...
  async (req, res) => {
    try {
      ...
      if (
        ...
        !(
          req.usuario.vacunas.general?.gestion === 1 ||
          req.usuario.vacunas.gestion?.includes(body.destino)
        )
      ) {
        return errorMessage(res, {message: "Acceso Denegado."}, 401);
      }
      ...
      if (!body._id) {
        ...
        if (body.remito_compra !== "Carga inicial") {
          ...
          ingresoDB = await VacunaIngreso.findOne({remito_compra: body.remito_compra}).exec();
          if (ingresoDB) {
            return errorMessage(res, {message: "Remito de Compra existente."}, 401);
          }
        }
        ingresoDB = await new VacunaIngreso(body).save();
      } else {
        // Update
        // Verificar que no se hayan recibido todos los insumos. (todos recibidos)
        ingresoDB = await VacunaIngreso.findOne({_id: body._id}).exec();
...
app.delete(
  ...
  async (req, res) => {
    try {
      let ingresoBorrado = await VacunaIngreso.findOne({_id: req.params.id}).exec();

      if (
        // verificar que sea general.gestion o que "destino" sea de su gestion.
        !(
          req.usuario.vacunas.general?.gestion === 1 ||
          req.usuario.vacunas.gestion?.includes(ingresoBorrado.destino.toString())
        )
      ) {
        return errorMessage(res, {message: "Acceso Denegado."}, 401);
      }
      ...
      ingresoBorrado = await VacunaIngreso.findOneAndDelete({_id: req.params.id}).exec();
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // Transferencias Remitos (clearing)
      let transferenciasDB = await VacunaTransferencia.aggregate()
        .match({
          $and: [
            {
              $or: [
  ...
  async (req, res) => {
    try {
      ...
      if (!body._id) {
        ...
        let ultimaTransferencia = await VacunaTransferencia.findOne({})
          .select({remito: 1, _id: 0})
          .sort({_id: -1})
          .exec();
        // si existe y es del año actual
        if (ultimaTransferencia?.remito.split("-")[0] === remitoBase[0]) {
        ...
        transferenciaDB = await new VacunaTransferencia(body).save();
      } else {
        // Update
        transferenciaDB = await VacunaTransferencia.findOne({_id: body._id}).exec();
...
app.delete(
  ...
  async (req, res) => {
    try {
      let transferenciaBorrada = await VacunaTransferencia.findOne({_id: req.params.id}).exec();

      if (
        // verificar que sea general.gestion o que "origen" sea de su gestion.
        !(
          req.usuario.vacunas.general?.gestion === 1 ||
          req.usuario.vacunas.gestion?.includes(transferenciaBorrada.origen.toString())
        )
      ) {
        return errorMessage(res, {message: "Acceso Denegado."}, 401);
      }
      ...
      transferenciaBorrada = await VacunaTransferencia.findOneAndDelete({
        _id: req.params.id,
      }).exec();
